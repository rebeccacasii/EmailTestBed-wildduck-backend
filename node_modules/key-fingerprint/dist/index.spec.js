"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const _1 = require(".");
jasmine.DEFAULT_TIMEOUT_INTERVAL = 5000;
// tslint:disable-next-line:no-debugger
debugger;
const algs = [
    'md4',
    'md5',
    'rmd160',
    'sha1',
    'sha224',
    'sha256',
    'sha384',
    'sha512',
];
const encs = [
    'hex',
    'base64',
];
// tslint:disable-next-line:max-line-length
const cert = '-----BEGIN CERTIFICATE----------END CERTIFICATE-----';
beforeAll(() => void 0);
afterAll(() => void 0);
describe('key-fingerprint', () => {
    describe('algorithms', () => {
        it('default', () => {
            expect(_1.fingerprint(cert)).toMatchSnapshot();
        });
        for (const alg of algs) {
            it(alg, () => {
                expect(_1.fingerprint(cert, alg)).toMatchSnapshot();
            });
            it(alg + ' with colons', () => {
                expect(_1.fingerprint(cert, alg, true)).toMatchSnapshot();
            });
        }
        for (const alg of algs) {
            for (const enc of encs) {
                it(alg + ' with ' + enc + ' encoding', () => {
                    expect(_1.fingerprint(cert, { algorithm: alg, encoding: enc })).toMatchSnapshot();
                });
                it(alg + ' with ' + enc + ' encoding and with colons', () => {
                    expect(_1.fingerprint(cert, { algorithm: alg, encoding: enc, colons: true })).toMatchSnapshot();
                });
            }
        }
    });
});
//# sourceMappingURL=index.spec.js.map