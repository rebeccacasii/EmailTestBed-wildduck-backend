...
module.exports = (db, server) => {
    server.put(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            const schema = Joi.object().keys({
                ...
                goal: Joi.string().length(1).trim().required(),
                voice: Joi.string().length(1).trim().required(),
                subject: Joi.string().min(1).max(100).trim().required(),
                body: Joi.string().min(1).max(500).trim().required()
            });
            ...
            const query = {subject: result.value.subject}
            const update = result.value
            const r = await db.users.collection('etb-message').updateOne(query, {$set: update}, { upsert: true });
            res.json({
                success: true
            });
    ...
    server.get(
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            let entry = await db.users.collection('etb-message').find();
            entry = entry || {};
